...
module.exports = {
    view: function (vnode) {
        ...
        if ((!firebase.loggedIn) && vnode.attrs.filter.isMyLectures) {
            //console.warn('You need to be logged in for this tab to work.')
        ...
        if (firebase.loggedIn && vnode.attrs.filter.isMyLectures && !StateLectures.receivedMyLectures) {
            //console.log('Waiting for your lectures from the database...');
        ...
        if (!vnode.attrs.filter.isMyLectures && !StateLectures.receivedLectures) {
            //console.log('Waiting for all lectures to be received...');
            return m(spinner);
        }

        //console.log(StateLectures.lectures);
        return [
            m(LectureHeader, {filter: vnode.attrs.filter}),
            m('.lecture-list', vnode.attrs.lectures.map(function (lecture) {
                return m('.list-item-wrapper', m(LectureListItem, {
                    lid: vnode.attrs.lid,
                    lecture: lecture,
                    isMyLectures: vnode.attrs.filter.isMyLectures
                }))
            }))]
    }
};